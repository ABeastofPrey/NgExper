...
export class VisionCommandComponent implements OnInit, OnDestroy {
    ...
    public createVar(type: CommandOptions): void {
        const option = {
            hasBackdrop: false,
            data: {}
        };
    ...
    private getCommand(type: CommandType, option: GetCommandOption): string {
        ...
        switch (type) {
            ...
            case CommandType.GetJobError:
                ...
                break;
            case CommandType.GetJobSatus:
                command = `${CommandType.GetJobSatus}("${option[CommandOptions.Station]}", "${option[CommandOptions.Job]}", "${status}", ${option[CommandOptions.Timeout]})`;
                break;
        ...
        command = !option[CommandOptions.Variable] ? `?${command}` : `${option[CommandOptions.Variable]} = ${command}`;
        return command;
    }
